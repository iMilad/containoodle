...
<img src="firefox-extension/icons/icon.svg" width="96" height="96" alt="Orbiting Turnip logo">

# Orbiting Turnip

**AWS console sessions, neatly separated.**

One click from the Orbiting Turnip sidebar or your AWS Access Portal opens each AWS account in
...
![Firefox](https://img.shields.io/badge/Firefox-MV3%20%C2%B7%20%E2%89%A5140-orange)
...
for up to five identities in one browser. Orbiting Turnip is aimed at workflows that need
...
## What Orbiting Turnip does

Orbiting Turnip puts every AWS account in a dedicated [Firefox container](https://support.mozilla.org/kb/containers)
so AWS console sessions do not overwrite one another. Use your local AWS CLI SSO
session when the Access Portal is unavailable, or let Orbiting Turnip hand launches from
the reachable portal into the right container.

| Capability | Behaviour |
|---|---|
| 🎯 **One click in** | Backend mode launches from the Orbiting Turnip sidebar; portal mode lets you choose the account and role in the AWS Access Portal as usual. A sidebar button focuses or opens the configured portal. Either way, the console lands in the account's container. |
...
| 🔒 **Local helper** | The optional backend binds to `127.0.0.1`, generates URLs from your local SSO cache, and has no hosted Orbiting Turnip service or telemetry. Its session-generation calls go to AWS. |

> **Why "Orbiting Turnip"?** No serious reason. It is memorable, unrelated, and the turnip gets a tiny orbit in the icon.
...
federation request still go to AWS; Orbiting Turnip has no hosted backend or telemetry
service.
...
unavailable, Orbiting Turnip uses an existing portal pin or a previously captured portal name
...
profile. The URL is never baked into the source or sent to an Orbiting Turnip service.
...
URL requests its exact origin and, after a successful change, removes Orbiting Turnip's
previous exact-origin grant. Revoking a host permission stops future Orbiting Turnip access
...
freeze the extension. **Reset existing titles to automatic** removes stored Orbiting Turnip
...
Orbiting Turnip requires **Firefox 140 or newer** because it uses Firefox tab groups and
Firefox's built-in data-consent disclosure. Download the appropriate asset from the
[latest release](../../releases/latest):

- Prefer `orbiting-turnip-<version>-signed.xpi` when it is present; it installs permanently on regular Firefox.
- The unsigned `orbiting-turnip-<version>.xpi` can be loaded temporarily through `about:debugging` → **This Firefox** → **Load Temporary Add-on**. It is removed when Firefox restarts.
...
Open the sidebar with **View → Sidebar → Orbiting Turnip**, `Alt+Shift+A`, or
...
1. Open Orbiting Turnip options and select **AWS access portal**.
...
6. Choose an account and role in the AWS portal normally. Orbiting Turnip hands that launch into the matching container and tab group.
...
choices for pinned accounts** only if you want Orbiting Turnip to load or change a pin's
...
1. Clone the public repository:
   ...
   ```bash
   git clone <repository-url> orbiting-turnip
   cd orbiting-turnip
   ```
...
4. Start the helper:
   ...
   ```bash
   python3 server.py
   # Orbiting Turnip is now available at http://127.0.0.1:8421
   ```

5. In Orbiting Turnip options select **Local AWS CLI helper**, keep or change the helper URL,
...
For backend sidebar launches, Orbiting Turnip checks an explicit backend choice, the account's
...
`ORBITING_TURNIP_DEFAULT_ROLE` (`AdministratorAccess` by default). A configured or selected
...
- **The helper reports that the token expires within five minutes:** run `aws sso login` again. Orbiting Turnip never moves or deletes the AWS CLI cache to refresh it.
- **AWS CLI rejects a backend role:** select a permission set available to that account, or correct the role in `~/.aws/accounts.json` / `ORBITING_TURNIP_DEFAULT_ROLE`.
...
- **A tab-group regex does not affect an older group:** use **Reset existing titles to automatic**. This intentionally removes stored manual Orbiting Turnip group-title overrides before recalculating them.
...
- The backend binds to `127.0.0.1` and has no authentication. For account and session endpoints, requests carrying an `Origin` header are accepted from Orbiting Turnip extension origins; requests without `Origin` are deliberately available to command-line clients. Any local process that can reach the port can call it, so run it only on a trusted workstation and never expose or forward the port.
- The helper builds session sign-in URLs in memory. It does not write them to Orbiting Turnip files or include query strings in its access log; Firefox may retain navigated URLs according to its own history and session policies.
...
- Host permissions are **opt-in at runtime and independently revocable**: core portal setup asks only for the exact configured portal origin. Portal API access is optional and used only for role choices on portal pins. AWS console-cookie access is optional and used only for backend session reuse/banner suppression. Orbiting Turnip has no hosted backend and sends no analytics or telemetry to an Orbiting Turnip service.
- Orbiting Turnip's complete data handling, retention, and deletion terms are in the [privacy policy](PRIVACY.md). Firefox's install prompt discloses the data categories the extension handles even though none of that data is sent to the developer.
...
- **Portal mode trade-off, deliberately accepted:** the `x-amz-sso_authn` cookie can mint console sessions for every account your SSO user is entitled to. Orbiting Turnip copies its Firefox domain/path/isolation scope into each launched account's container and verifies the copy before navigation; Firefox may retain it until its original expiry. Optional sidebar role discovery also sends its value as a bearer token directly to the regional AWS portal API. Orbiting Turnip never writes the value to extension storage or logs.
...
[MIT](LICENSE) © Orbiting Turnip contributors
